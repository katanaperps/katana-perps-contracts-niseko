...
describe('Exchange', function () {
  ...
  describe('invalidateNonce', async function () {
    ...
    it('should work on subsequent valid call', async function () {
      await exchange.connect(traderWallet).invalidateNonce(
        uuidToHexString(
          uuidv1({
            msecs: (await getLatestBlockTimestampInSeconds()) * 1000 - 100,
          }),
        ),
      );
    ...
    it('should revert for wrong UUID version', async function () {
      ...
      ).to.be.revertedWith(/must be v1 uuid/i);
    });
    ...
    it('should revert for timestamp too far in the future', async function () {
      ...
      ).to.be.revertedWith(/nonce timestamp too high/i);
    });
    ...
    it('should revert on invalidating same timestamp twice', async function () {
      ...
      ).to.be.revertedWith(/nonce timestamp invalidated/i);
    });
    ...
    it('should revert on subsequent call before block threshold of previous', async () => {
      ...
      await exchange.connect(traderWallet).invalidateNonce(
        uuidToHexString(
          uuidv1({
            msecs: (await getLatestBlockTimestampInSeconds()) * 1000 - 10,
          }),
        ),
      );

      await expect(
        exchange.connect(traderWallet).invalidateNonce(
          uuidToHexString(
            uuidv1({
              msecs: (await getLatestBlockTimestampInSeconds()) * 1000 + 10,
            }),
      ...
      ).to.be.revertedWith(/last invalidation not finalized/i);
    });
  });
