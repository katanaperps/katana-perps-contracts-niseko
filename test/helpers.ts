...
import type { IndexPrice, Order, Trade } from '../lib';
import type { Exchange_v1, Governance, USDC } from '../typechain-types';
...
export async function deployContractsExceptCustodian(
  ...
  const [
    ...
    ethers.getContractFactory('KatanaPerpsIndexAndOraclePriceAdapter'),
  ...
  const indexPriceAdapter = await (
    await KatanaPerpsIndexAndOraclePriceAdapterFactory.connect(owner).deploy(
      owner.address,
