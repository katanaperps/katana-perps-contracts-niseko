...
import { Balance, NonceInvalidation, WalletExit } from "../libraries/Structs.sol";
...
interface IExchangeExtended is IExchange {
  ...
  function loadWalletExitStatus(address wallet) external view returns (WalletExit memory walletExit);
}
...
contract ExchangeWalletStateAggregator {
  ...
  function loadWalletStates(address[] memory wallets) public view returns (WalletState[] memory walletStates) {
    ...
    for (uint256 i = 0; i < wallets.length; i++) {
      ...
      walletStates[i].walletExit = exchange.loadWalletExitStatus(wallets[i]);
