...
contract Governance is Owned {
  ...
  ManagedAccountProvidersUpgrade public currentManagedAccountProvidersUpgrade;
  ...
  struct ManagedAccountProvidersUpgrade {
    bool exists;
    IManagedAccountProvider[] newManagedAccountProviders;
    uint256 blockTimestampThreshold;
  }
  ...
  /**
   * @notice Emitted when admin initiates Managed Account Providers upgrade with
   * `initiateManagedAccountProvidersUpgrade`
   */
  event ManagedAccountProvidersUpgradeInitiated(
    IManagedAccountProvider[] newManagedAccountProviders,
    uint256 blockTimestampThreshold
  );
  /**
   * @notice Emitted when admin cancels previously started Managed Account Providers upgrade with
   * `cancelManagedAccountProvidersUpgrade`
   */
  event ManagedAccountProvidersUpgradeCanceled();
  /**
   * @notice Emitted when admin finalizes Managed Account Providers upgrade with
   * `finalizeManagedAccountProvidersUpgrade`
   */
  event ManagedAccountProvidersUpgradeFinalized(IManagedAccountProvider[] newManagedAccountProviders);
  /**
  ...
  modifier onlyExchangeAdmin() {
    require(msg.sender == Owned(address(_loadExchange())).adminWallet(), "Caller must be Exchange Admin");
    _;
  }

  modifier onlyExchangeAdminOrDispatcher() {
    require(
      msg.sender == Owned(address(_loadExchange())).adminWallet() || msg.sender == _loadExchange().dispatcherWallet(),
      "Caller must be Exchange Admin or Dispatcher wallet"
    );
    _;
  }
  ...
  /**
   * @notice Initiates Managed Account Providers upgrade process. Once block delay has passed the
   * process can be* finalized with `finalizeManagedAccountProvidersUpgrade`
   *
   * @param newManagedAccountProviders The new Managed Account Provider contract addresses. If the
   * Exchange has any current Managed Account Provider addresses whitelisted, these must be included
   * in the arguments in matching order
   */
  function initiateManagedAccountProvidersUpgrade(
    IManagedAccountProvider[] memory newManagedAccountProviders
  ) public onlyAdmin {
    require(!currentManagedAccountProvidersUpgrade.exists, "Managed Account Providers upgrade already in progress");

    uint8 currentManagedAccountProvidersLength = SafeCast.toUint8(_loadExchange().loadManagedAccountProvidersLength());

    for (uint8 i = 0; i < newManagedAccountProviders.length; i++) {
      if (i < currentManagedAccountProvidersLength) {
        require(
          newManagedAccountProviders[i] == _loadExchange().loadManagedAccountProvider(i),
          "Current Managed Account Provider addresses must be included"
        );
      }
      require(Address.isContract(address(newManagedAccountProviders[i])), "Invalid Managed Account Provider address");
    }

    currentManagedAccountProvidersUpgrade.exists = true;
    currentManagedAccountProvidersUpgrade.newManagedAccountProviders = newManagedAccountProviders;
    currentManagedAccountProvidersUpgrade.blockTimestampThreshold =
      block.timestamp +
      Constants.FIELD_UPGRADE_DELAY_IN_S;

    emit ManagedAccountProvidersUpgradeInitiated(
      newManagedAccountProviders,
      currentManagedAccountProvidersUpgrade.blockTimestampThreshold
    );
  }

  /**
   * @notice Cancels an in-flight Managed Account Providers upgrade that has not yet been finalized
   */
  function cancelManagedAccountProvidersUpgrade() public onlyAdmin {
    require(currentManagedAccountProvidersUpgrade.exists, "No Managed Account Providers upgrade in progress");

    delete currentManagedAccountProvidersUpgrade;

    emit ManagedAccountProvidersUpgradeCanceled();
  }

  /**
   * @notice Finalizes the Managed Account Providers upgrade. The number of seconds specified by
   * `Constants.FIELD_UPGRADE_DELAY_IN_S` must have passed since calling `initiateManagedAccountProvidersUpgrade`
   *
   * @param newManagedAccountProviders The new Managed Account Provider contract addresses. Must
   * match the order and values of the addresses provided to `initiateManagedAccountProvidersUpgrade`
   */
  function finalizeManagedAccountProvidersUpgrade(
    IManagedAccountProvider[] memory newManagedAccountProviders
  ) public onlyAdminOrDispatcher {
    require(currentManagedAccountProvidersUpgrade.exists, "No Managed Account Providers upgrade in progress");

    require(
      block.timestamp >= currentManagedAccountProvidersUpgrade.blockTimestampThreshold,
      "Block timestamp threshold not yet reached"
    );

    // Verify provided addresses match originals
    require(
      currentManagedAccountProvidersUpgrade.newManagedAccountProviders.length == newManagedAccountProviders.length,
      "Address mismatch"
    );
    for (uint8 i = 0; i < newManagedAccountProviders.length; i++) {
      require(
        currentManagedAccountProvidersUpgrade.newManagedAccountProviders[i] == newManagedAccountProviders[i],
        "Address mismatch"
      );
    }

    _loadExchange().setManagedAccountProviders(currentManagedAccountProvidersUpgrade.newManagedAccountProviders);

    delete currentManagedAccountProvidersUpgrade;

    emit ManagedAccountProvidersUpgradeFinalized(newManagedAccountProviders);
  }
  ...
  /**
   * @notice Initiates market override upgrade process for the Insurance Fund wallet currently
   * whitelisted by the Exchange. Once `Constants.FIELD_UPGRADE_DELAY_IN_S` has passed the process
   * can be finalized with `finalizeMarketOverridesUpgradeForInsuranceFundWallet`
   *
   * @param baseAssetSymbol The base asset symbol for the market
   * @param overridableFields New values for overridable fields
   * @param wallet The wallet to apply overrides to. Must match the Insurance Fund wallet currently
   * whitelisted by the Exchange
   */
  function initiateMarketOverridesUpgradeForInsuranceFundWallet(
    string memory baseAssetSymbol,
    OverridableMarketFields memory overridableFields,
    address wallet
  ) public onlyExchangeAdmin {
    require(wallet == _loadExchange().insuranceFundWallet(), "Wallet must be currently whitelisted IF wallet");
    require(
      !currentMarketOverridesUpgradesByBaseAssetSymbolAndWallet[baseAssetSymbol][wallet].exists,
      "Market override upgrade already in progress for IF wallet"
    );
  ...
  /**
   * @notice Cancels an in-flight market override upgrade process for the Insurance Fund wallet
   * currently whitelisted by the Exchange that has not yet been finalized
   */
  function cancelMarketOverridesUpgradeForInsuranceFundWallet(
    string memory baseAssetSymbol,
    address wallet
  ) public onlyExchangeAdmin {
    require(wallet == _loadExchange().insuranceFundWallet(), "Wallet must be currently whitelisted IF wallet");
  ...
  /**
   * @notice Finalizes a market override upgrade process by assigning wallet-specific overrides to
   * the Insurance Fund wallet currently whitelisted byt he Exchange. The number of seconds
   * specified by`Constants.FIELD_UPGRADE_DELAY_IN_S` must have passed since calling
   * `initiateMarketOverridesUpgradeForInsuranceFundWallet`
   */
  function finalizeMarketOverridesUpgradeForInsuranceFundWallet(
    string memory baseAssetSymbol,
    OverridableMarketFields memory overridableFields,
    address wallet
  ) public onlyExchangeAdminOrDispatcher {
    require(wallet == _loadExchange().insuranceFundWallet(), "Wallet must be currently whitelisted IF wallet");
    MarketOverridesUpgrade memory upgrade = currentMarketOverridesUpgradesByBaseAssetSymbolAndWallet[baseAssetSymbol][
      wallet
    ];
